...
vecRow <- seq(1, nrow(lsHyperparam$dfInteraction))
# cluster function depending on rowid
ClusterFunction <- function(rowid){
  tryCatch(expr = {
    ## get hyperparam
    step <- as.character(lsHyperparam$dfInteraction[rowid, "step"])
    span <- as.numeric(lsHyperparam$dfInteraction[rowid, "span"])
    model <- lsHyperparam$dfInteraction[rowid, "model"]
    df <- as.character(lsHyperparam$dfInteraction[rowid, "df"])
    forecast <- as.numeric(lsHyperparam$dfInteraction[rowid, "forecast"])
    rolderiv <- lsHyperparam$dfInteraction[rowid, "rolderiv"]
    date <- as.Date(lsHyperparam$dfInteraction[rowid, "Date"])
    stepID <- as.numeric(lsHyperparam$dfInteraction[rowid, "STEP_ID"])
...
subDir <- paste0("PredictCovid_data/results_simu/multisteps_publication_interaction", slar_jobid)
